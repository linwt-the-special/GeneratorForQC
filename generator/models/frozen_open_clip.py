...
class FrozenOpenCLIPEmbedder(Config_Model):
    """Loads and freezes the [OpenCLIP](https://github.com/mlfoundations/open_clip) transformer encoder for text prompts.
       Probably unable to download if you are in China. Please download it from other resources and set the version to the local file address.
    """
    ...
    def __init__(self, arch=None, version="laion2b_s32b_b79k", device="cpu", max_length=77, freeze=True, layer="penultimate"):
        super().__init__()               
        assert layer in self.LAYERS     
        self.params_config = FrozenOpenCLIPEmbedder_config(arch, version, device, max_length, freeze, layer)
        
        if not arch:
            model, _, _ = open_clip.create_model_and_transforms(device=torch.device(device), pretrained=version)
        else:
