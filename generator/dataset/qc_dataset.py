...
from ..platform.simulation.qcircuit_sim import schmidt_rank_vector, instruction_name_to_gate
...
class Qc_Config_Dataset(Config_Dataset):
    ...
    def plot_example(self):
        ...
        if isinstance(self.gate_pool[0], str):
            gate_pool = [instruction_name_to_gate(gate) for gate in self.gate_pool]
        else:
        ...
        print(f"Label: ``{label}``    SRV is: {schmidt_rank_vector(simulator.run(qc))}")
        display(qc.draw(filename='output'))
